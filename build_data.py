...
def generate_data_dict(xlsx_file_name,sheet_name, tuh_eeg_szr_ver):
    ...
    for item in train_files:
        ...
        seizure_inf = seizure_info(patient_id = patient_id, filename = item[0], start_time=item[1], end_time=item[2])
        seizure_type = item[3]  # szr_type
        data_dict[seizure_type].append(seizure_inf)
...
def print_type_information(data_dict):
    ...
    for szr_type, szr_info_list in data_dict.items():
        ...
        list.append([szr_type, (len(szr_info_list)), (len(unique_patient_id_list)), (total_dur)])

    sorted_by_szr_num = sorted(list, key=lambda tup: tup[1], reverse=True)
...
def load_edf_extract_seizures_v152(base_dir, save_data_dir, data_dict):
    ...
    count = 0
    bar = progressbar.ProgressBar(maxval=sum(len(v) for k, v in data_dict.items()),widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
    bar.start()
    for seizure_type, seizures in data_dict.items():
        number = 0
        if seizure_type != 'MYSZ':
...
def gen_raw_seizure_pkl(args,tuh_eeg_szr_ver, anno_file):
    ...
    #print(raw_data_base_dir)
    szr_annotation_file = os.path.join(base_dir+"/", '_DOCS/', anno_file)
    #print(szr_annotation_file)
    ...
    if tuh_eeg_szr_ver == 'v1.5.2':
        seizure_data_dict = load_edf_extract_seizures_v152(base_dir, save_data_dir, merged_dict)
    else:
        exit('tuh_eeg_szr_ver %s is not supported' % tuh_eeg_szr_ver)
...
def main():
    ...
    if platform.system() == 'Linux':
        ...
        parser.add_argument('--base_dir', default='D:/datasets',
                            help='path to raw seizure dataset')
        parser.add_argument('--save_data_dir',
                            default='D:/datasets/seizure_result_data',
                            help='path to save processed data')
    else:
