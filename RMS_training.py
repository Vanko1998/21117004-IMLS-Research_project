...
def train_model(data_dir, fold_data, labelEncoder, model_path, logs_dir, acc_per_fold, loss_per_fold):
...
def whole_process(cross_val_file, test_file, data_dir, sr, wl):
    ...
    for fold_no, fold_data in enumerate(k_validation_folds):
        ...
        acc_per_fold, loss_per_fold, model, x_train, y_train, x_val, y_val, time_t = train_model(data_dir, fold_data, le,
                                                                                         "./RMS_test_model_sr_"+str(sr)+"_wl_"+str(wl), logs_dir,
                                                                                         acc_per_fold, loss_per_fold)
        totaltime.append(time_t)
    if not os.path.exists("./RMS_test_model_sr_"+str(sr)+"_wl_"+str(wl)):
        model.save("./RMS_test_model_sr_"+str(sr)+"_wl_"+str(wl))
...
def bar_plot(acc_train, acc_valid, acc_test, x_axis_all):
    ...
    # According to legend
    plt.title("RMS_train_validation_test_accuracy")
...
def main():
    ...
    for sr in sampling_frequency:
        for wl in window_length:
            start = time.perf_counter()
            data_dir = "D:/datasets/seizure_preprocessed_data(spectograms)/v1.5.2/fft/RMS_fft_seizures_wl_"+str(wl)+"_ws_0.75_sf_"+str(sr)+"_fft_min_1_fft_max_48"  # "/home/david/Documents/Machine Learning/raw_data/fft_seizures_wl1_ws_0.5_sf_250_fft_min_1_fft_max_12"
            cross_val_file = "rms_wl_"+str(wl)+"_ws_0.75_sf_"+str(sr)+"_cv_split_5_fold_seizure_wise_v1.5.2.pkl"
            test_file = "rms_wl_"+str(wl)+"_ws_0.75_sf_"+str(sr)+"_cv_split_5_fold_seizure_wise_v1.5.2_test.pkl"
            ...
            avr_time.append(avr)
            all_time.append(time.perf_counter()-start)
    np.savetxt('./RMS_pred_time.txt', pred_time)
    np.savetxt('./RMS_avr_evaluate_time.txt', avr_time)
    np.savetxt('./RMS_test_acc.txt', acc_test)
    np.savetxt('./RMS_x_axis.txt', x_axis_all, fmt="%s", delimiter="")
    np.savetxt('./RMS_all_time.txt', all_time)
